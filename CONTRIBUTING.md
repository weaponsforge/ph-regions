...
Welcome to the **ph-regions** repository! We're excited to have you contribute to creating a well-structured, scalable, and fully documented REST API that delivers **test/mock data for the Philippines' administrative divisions** - including regions, provinces, cities, municipalities, and barangays, ideal for testing and development purposes.
...
2. **Create a Pull Request (PR):** Go to the original **ph-regions** repository on GitHub and create a new pull request. Base your PR on your feature branch and target the `"dev"` branch of the upstream repository.
...
2. **Relevance:** Content should be relevant to REST API development and testing using Express and TypeScript.
...
4. **Respect:** Respect the structure and formatting of the existing project. Follow the standard ESLint rules defined in its `server/eslint.config.mjs` file.

Thank you for contributing to the **ph-regions** repository. Your efforts help in making test Philippine location data available for testing RESTful API consumption.